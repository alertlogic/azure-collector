...
class O365Management extends msRestAzure.AzureServiceClient {
    ...
    subscriptionsList(options, callback) {
        ...
        if (!this.generateClientRequestId) {
            options.disableClientRequestId = true;
        }
        ...
        // Send Request
        return client.sendRequest(httpRequest, (err, responseBody, request, response) => {
            if (err) {
                return callback(err);
            }
            ...
            if (statusCode !== 200) {
                ...
                if (responseBody) {
                    if (responseBody.error) responseBody = responseBody.error;
                    if (responseBody.code) error.code = responseBody.code;
                    if (responseBody.message) error.message = responseBody.message;
                }
                return callback(JSON.stringify(error));
            }
            return callback(null, responseBody, httpRequest, response);
        });
    }
