...
class O365Management extends msRestAzure.AzureServiceClient {
    ...
    requestHandler(httpRequest, callback){
        return (res) => {
            let {parsedBody, status} = res;
            if (status !== 200) {
            ...
            return callback(null, parsedBody, httpRequest, res);
        }
    }
    
    subscriptionsList(options, callback) {
        console.log('trying subscriptions list')
        let client = this;
        // Construct URL
        ...
        // Create handler
        const handler = this.requestHandler(httpRequest, callback);
        // Send Request
        return client.sendRequest(httpRequest).then(handler).catch((err) => callback(err));
    }
